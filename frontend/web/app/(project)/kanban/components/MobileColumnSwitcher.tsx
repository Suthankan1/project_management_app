...
export default function MobileColumnSwitcher({
  ...
  return (
    <div className="md:hidden flex overflow-x-auto no-scrollbar gap-1 mb-3 bg-cu-bg rounded-xl p-1 border border-cu-border shadow-sm">
      {columnConfigs.filter(col => col && col.status).map((col) => (
        <button
          ...
          className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all whitespace-nowrap ${activeMobileColumn === col.status
              ...
              : 'text-cu-text-tertiary hover:text-cu-text-primary'
            }`}
        >
          {col.title}
          <span className={`ml-1.5 text-[10px] rounded-full px-1.5 py-0.5 ${activeMobileColumn === col.status ? 'bg-white/20 text-white' : 'bg-cu-bg-secondary text-cu-text-tertiary'
            }`}>
