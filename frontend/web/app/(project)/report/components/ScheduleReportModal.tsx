...
function StepDot({ n, current, label, isDark }: { n: number; current: number; label: string; isDark: boolean }) {
    ...
    <div className="flex flex-col items-center gap-1">
      <div
        ...
        style={{
          ...
          color: done || active ? '#fff' : (isDark ? '#64748B' : '#9CA3AF'),
          boxShadow: active ? '0 0 0 4px rgba(21,93,252,0.2)' : 'none',
        }}
      >
        {done ? <CheckCircle2 size={13} /> : n}
      </div>
      <span className="text-[9px] font-bold uppercase tracking-wider" style={{ color: active ? '#155DFC' : (isDark ? '#475569' : '#9CA3AF') }}>
        {label}
      </span>
    </div>
  );
      ...
      {options.map(o => {
        ...
        return (
          <motion.button
            ...
            style={{
              borderColor: sel ? '#155DFC' : (isDark ? '#273449' : '#E5E7EB'),
              background:  sel ? (isDark ? 'rgba(21,93,252,0.12)' : '#EBF2FF') : (isDark ? '#0F1724' : '#FFFFFF'),
            }}
          >
            {o.Icon && (
              <div
                className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
                style={{ background: sel ? '#155DFC' : (isDark ? '#1E293B' : '#F3F4F6') }}
              >
                <o.Icon size={17} style={{ color: sel ? '#fff' : (isDark ? '#94A3B8' : '#6B7280') }} />
              </div>
            )}
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-bold" style={{ color: sel ? '#155DFC' : (isDark ? '#F1F5F9' : '#1F2937') }}>{o.label}</p>
              {o.desc && <p className="text-[11px] mt-0.5" style={{ color: isDark ? '#475569' : '#9CA3AF' }}>{o.desc}</p>}
            </div>
            <div
              className="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0"
              style={{
                borderColor: sel ? '#155DFC' : (isDark ? '#334155' : '#D1D5DB'),
                background:  sel ? '#155DFC' : 'transparent',
              }}
            >
...
function FieldLabel({ children, isDark }: { children: React.ReactNode; isDark: boolean }) {
  return (
    <label className="text-[11px] font-bold uppercase tracking-widest mb-1.5 block" style={{ color: isDark ? '#64748B' : '#6B7280' }}>
      {children}
    </label>
  );
}
...
function TextInput({
  ...
  return (
    <input
      ...
      className="w-full h-10 px-3 rounded-xl border text-[12px] outline-none transition-all focus:ring-2 focus:ring-[#155DFC]/10 focus:border-[#155DFC]"
      style={{
        background: isDark ? '#0F1724' : 'rgba(255,255,255,0.8)',
...
function SelectInput({
  value, onChange, children, isDark,
}: {
  ...
  return (
    <select
      value={value}
      onChange={e => onChange(e.target.value)}
      className="w-full h-10 px-3 rounded-xl border text-[12px] outline-none transition-all cursor-pointer focus:ring-2 focus:ring-[#155DFC]/10 focus:border-[#155DFC]"
      style={{
        background: isDark ? '#0F1724' : 'rgba(255,255,255,0.8)',
        border:     `1px solid ${isDark ? '#273449' : '#E5E7EB'}`,
        color:      isDark ? '#F1F5F9' : '#1F2937',
...
function SummaryRow({ label, value, isDark }: { label: string; value: string; isDark: boolean }) {
  return (
    <div className="flex items-start gap-3 py-2 last:border-0" style={{ borderBottom: `1px solid ${isDark ? '#1E293B' : '#F3F4F6'}` }}>
      <span className="text-[10px] font-bold uppercase tracking-wider w-24 shrink-0 pt-0.5" style={{ color: isDark ? '#64748B' : '#9CA3AF' }}>{label}</span>
      <span className="text-[12px] font-semibold flex-1 break-all" style={{ color: isDark ? '#F1F5F9' : '#1F2937' }}>{value || '—'}</span>
    </div>
  );
}
...
export default function ScheduleReportModal({ open, onClose, projectId, projectName }: Props) {
  ...
  const validate = (s: number): boolean => {
    ...
    if (s === 4 && form.scheduleType === 'RECURRING') {
      ...
      if (form.endType === 'UNTIL_DATE' && !form.endDate) errs.endDate = 'Please pick an end date.';
    }
  ...
  const resetAndClose = useCallback(() => {
    setStep(1); setForm(EMPTY_FORM); setErrors({}); setSaveState('idle'); onClose();
  }, [onClose]);
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      setSaveState('done');
      setTimeout(() => { setSaveState('idle'); resetAndClose(); }, 2500);
    } catch {
      setSaveState('error');
  ...
  function renderMessageStep() {
    return (
      <div className="flex flex-col gap-4">
        <div>
          <FieldLabel isDark={isDark}>Subject (optional)</FieldLabel>
          <TextInput
            value={form.subject}
        ...
        <div>
          <FieldLabel isDark={isDark}>Message / Body (optional)</FieldLabel>
          <textarea
            value={form.bodyMessage}
            onChange={e => set('bodyMessage', e.target.value)}
            placeholder="Add a personal note to the email…"
            rows={4}
            className="w-full px-3 py-2.5 rounded-xl border text-[12px] outline-none transition-all resize-none focus:ring-2 focus:ring-[#155DFC]/10 focus:border-[#155DFC]"
            style={{
              background: isDark ? '#0F1724' : 'rgba(255,255,255,0.8)',
              border:     `1px solid ${isDark ? '#273449' : '#E5E7EB'}`,
              color:      isDark ? '#F1F5F9' : '#1F2937',
  ...
  function renderSummary() {
    return (
      <div className="flex flex-col gap-3">
        <p className="text-[13px] font-bold mb-1" style={{ color: isDark ? '#F1F5F9' : '#1F2937' }}>Review your schedule</p>
        <div
          className="rounded-xl p-4"
          style={{
            background: isDark ? 'rgba(21,93,252,0.08)' : 'linear-gradient(135deg, rgba(21,93,252,0.05) 0%, rgba(77,139,255,0.04) 100%)',
            border:     `1px solid ${isDark ? 'rgba(21,93,252,0.2)' : 'rgba(21,93,252,0.12)'}`,
          }}
        >
          <SummaryRow isDark={isDark} label="Format"   value={formatLabel} />
          <SummaryRow isDark={isDark} label="Type"     value={form.scheduleType === 'ONE_TIME' ? 'One-Time' : 'Recurring'} />
          <SummaryRow isDark={isDark} label="Schedule" value={freqLabel} />
          {form.scheduleType === 'RECURRING' && <SummaryRow isDark={isDark} label="Ends" value={endLabel} />}
          <SummaryRow isDark={isDark} label="To"       value={form.recipientsTo.join(', ')} />
          {form.recipientsCc.length  > 0 && <SummaryRow isDark={isDark} label="CC"  value={form.recipientsCc.join(', ')}  />}
          {form.recipientsBcc.length > 0 && <SummaryRow isDark={isDark} label="BCC" value={form.recipientsBcc.join(', ')} />}
          {form.subject     && <SummaryRow isDark={isDark} label="Subject" value={form.subject} />}
          {form.bodyMessage && <SummaryRow isDark={isDark} label="Message" value={form.bodyMessage.slice(0, 80) + (form.bodyMessage.length > 80 ? '…' : '')} />}
        </div>
        <p className="text-[11px] text-center mt-1" style={{ color: isDark ? '#475569' : '#9CA3AF' }}>
  ...
  const renderStep = () => {
    switch (step) {
      case 1: return (
        <div className="flex flex-col gap-4">
          <div>
            <p className="text-[13px] font-bold mb-1" style={{ color: isDark ? '#F1F5F9' : '#1F2937' }}>How would you like to schedule this report?</p>
            <p className="text-[11px]" style={{ color: isDark ? '#475569' : '#9CA3AF' }}>Choose whether to send once or on a repeating schedule.</p>
          </div>
          <SegControl
            value={form.scheduleType} onChange={v => set('scheduleType', v)} isDark={isDark}
            options={[
      ...
      case 2: return (
        <div className="flex flex-col gap-5">
          <div>
            <FieldLabel isDark={isDark}>Report Format</FieldLabel>
            <div className="grid grid-cols-3 gap-2">
              {([ ['PDF','FileText','#DC2626','#FFF5F5','rgba(220,38,38,0.12)'], ['EXCEL','Table2','#16A34A','#F0FDF4','rgba(22,163,74,0.12)'], ['BOTH','Sparkles','#155DFC','#EBF2FF','rgba(21,93,252,0.12)'] ] as const).map(
                ([id, , color, bg, bgDk]) => {
                  ...
                  return (
                    <button key={id} type="button" onClick={() => set('format', id)}
                      ...
                      style={{
                        borderColor: sel ? color : (isDark ? '#273449' : '#E5E7EB'),
                        background:  sel ? (isDark ? bgDk : bg) : (isDark ? '#0F1724' : '#FAFAFA'),
                      }}
                    >
                      <Icon size={18} style={{ color }} />
                      <span className="text-[11px] font-bold" style={{ color: sel ? color : (isDark ? '#CBD5E1' : '#374151') }}>
                        {id === 'BOTH' ? 'Both' : id}
                      </span>
                    </button>
                  );
          ...
          {form.scheduleType === 'ONE_TIME' && (
            <div>
              <FieldLabel isDark={isDark}>Send Date</FieldLabel>
              <TextInput type="date" value={form.scheduledDate} onChange={v => set('scheduledDate', v)} isDark={isDark} />
          ...
          {form.scheduleType === 'RECURRING' && (
            <div className="flex flex-col gap-3">
              <div>
                <FieldLabel isDark={isDark}>Frequency</FieldLabel>
                <SelectInput value={form.frequency} onChange={v => set('frequency', v as Frequency)} isDark={isDark}>
              ...
              {form.frequency === 'CUSTOM' && (
                <div>
                  <FieldLabel isDark={isDark}>Every how many days?</FieldLabel>
                  <TextInput type="number" value={form.customIntervalDays} onChange={v => set('customIntervalDays', v)} placeholder="e.g. 3" isDark={isDark} />
                  {errors.customIntervalDays && <p className="text-[11px] text-red-500 mt-1">{errors.customIntervalDays}</p>}
                </div>
              )}
              {form.frequency === 'WEEKLY' && (
                <div>
                  <FieldLabel isDark={isDark}>Day of week</FieldLabel>
                  <SelectInput value={form.sendDayOfWeek} onChange={v => set('sendDayOfWeek', v)} isDark={isDark}>
              ...
              {form.frequency === 'MONTHLY' && (
                <div>
                  <FieldLabel isDark={isDark}>Day of month</FieldLabel>
                  <SelectInput value={form.sendDayOfMonth} onChange={v => set('sendDayOfMonth', v)} isDark={isDark}>
          ...
          <div>
            <FieldLabel isDark={isDark}>Time of Day (Local)</FieldLabel>
            <TextInput type="time" value={form.sendTime} onChange={v => set('sendTime', v)} isDark={isDark} />
      ...
      case 3: return (
        <div className="flex flex-col gap-4">
          <EmailChipInput label="To" required value={form.recipientsTo} onChange={v => set('recipientsTo', v)} error={errors.recipientsTo} placeholder="recipient@email.com" />
          <EmailChipInput label="CC"  value={form.recipientsCc}  onChange={v => set('recipientsCc', v)}  placeholder="cc@email.com" />
          <EmailChipInput label="BCC" value={form.recipientsBcc} onChange={v => set('recipientsBcc', v)} placeholder="bcc@email.com" />
        </div>
      );
      ...
      case 4:
        if (form.scheduleType === 'RECURRING') {
          return (
            <div className="flex flex-col gap-4">
              <div>
                <p className="text-[13px] font-bold mb-1" style={{ color: isDark ? '#F1F5F9' : '#1F2937' }}>When should the recurring schedule stop?</p>
                <p className="text-[11px]" style={{ color: isDark ? '#475569' : '#9CA3AF' }}>Configure the end condition for your repeating report.</p>
              </div>
              <SegControl
                value={form.endType} onChange={v => set('endType', v as EndType)} isDark={isDark}
                options={[
              ...
              {form.endType === 'AFTER_N' && (
                <div>
                  <FieldLabel isDark={isDark}>Number of sends</FieldLabel>
                  <TextInput type="number" value={form.endAfterCount} onChange={v => set('endAfterCount', v)} placeholder="e.g. 10" isDark={isDark} />
              ...
              {form.endType === 'UNTIL_DATE' && (
                <div>
                  ...
                  <TextInput type="date" value={form.endDate} onChange={v => set('endDate', v)} isDark={isDark} />
        ...
        <>
          ...
          >
            <div
              ...
              style={{
                background:     isDark ? 'rgba(11,17,32,0.97)' : 'rgba(255,255,255,0.93)',
            ...
            >
              ...
              {/* Step indicators */}
              <div
                className="px-6 pt-4 pb-3 flex items-center justify-between gap-2 shrink-0"
                style={{ borderBottom: `1px solid ${isDark ? '#1E293B' : '#F3F4F6'}` }}
              >
                {stepLabels.map((label, i) => (
                  <React.Fragment key={label}>
                    <StepDot n={i + 1} current={step} label={label} isDark={isDark} />
                    {i < stepLabels.length - 1 && (
                      <div className="flex-1 h-px" style={{ background: i + 1 < step ? '#16A34A' : (isDark ? '#273449' : '#E5E7EB') }} />
                    )}
                  </React.Fragment>
                ))}
              </div>
              ...
              <div
                ...
                style={{ borderTop: `1px solid ${isDark ? '#1E293B' : '#F3F4F6'}` }}
              >
                <button
                  onClick={back}
                  disabled={step === 1 || saveState === 'loading'}
                  className="flex items-center gap-1.5 px-4 h-9 rounded-xl text-[12px] font-semibold transition-colors disabled:opacity-40"
                  style={{ color: isDark ? '#94A3B8' : '#6B7280' }}
                  onMouseEnter={e => { if (!e.currentTarget.disabled) (e.currentTarget as HTMLElement).style.background = isDark ? '#1E293B' : '#F3F4F6'; }}
                ...
                <div className="flex items-center gap-1.5">
                  {stepLabels.map((_, i) => (
                    <div
                      ...
                      style={{ background: i + 1 === step ? '#155DFC' : (isDark ? '#273449' : '#E5E7EB') }}
                    />
                  ))}
                </div>
