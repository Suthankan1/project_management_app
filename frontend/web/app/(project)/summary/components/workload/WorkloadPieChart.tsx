...
const renderActiveShape = (props: ActiveShapeProps) => {
  ...
  const safeFill = fill || 'var(--cu-primary)';
  ...
  return (
    <div className="w-full lg:w-4/12 p-6 flex flex-col items-center justify-center relative" onMouseLeave={() => setActiveIndex(-1)}>
      {activeWorkloadData.length > 0 ? (
        <div className="relative w-full h-[280px]">
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none transition-all duration-300 z-10 opacity-100">
            {activeIndex === -1 ? (
              <>
                <h3 className="text-[24px] font-black text-cu-text-primary leading-none mb-1">{membersLength}</h3>
                <p className="text-[10px] font-bold text-cu-text-muted uppercase tracking-widest mb-1.5">Members</p>
                <div className="w-8 h-[2px] bg-cu-bg-tertiary rounded-full mb-1.5"></div>
                <h3 className="text-[18px] font-black text-cu-primary leading-none mb-1">{tasksLength}</h3>
                <p className="text-[10px] font-bold text-cu-text-muted uppercase tracking-widest">Tasks</p>
              </>
            ) : (
              <div className="flex flex-col items-center justify-center gap-2 mt-[-2px]">
                <h3 className="text-[22px] font-arimo font-extrabold tracking-tight text-cu-text-primary leading-[1.2] text-center px-4 line-clamp-1">
                  {activeWorkloadData[activeIndex]?.name === 'Unassigned' ? 'Unassigned' : activeWorkloadData[activeIndex]?.name?.split(' ')[0] || 'Unknown'}
                </h3>
                <p className="text-[13px] font-arimo text-cu-text-secondary">
        ...
        <div className="h-[280px] w-full flex flex-col items-center justify-center text-cu-text-muted">
          <Briefcase size={32} className="mb-2 opacity-40 text-cu-text-muted" />
      ...
      <Link
        ...
        className="group absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1.5 text-cu-text-primary bg-cu-bg/80 backdrop-blur-md border border-cu-border hover:bg-cu-primary hover:text-white hover:border-cu-primary rounded-xl px-4 py-2 font-arimo text-[12px] font-bold transition-all duration-300 shadow-cu-sm hover:shadow-cu-lg active:scale-[0.97] z-20"
      >
