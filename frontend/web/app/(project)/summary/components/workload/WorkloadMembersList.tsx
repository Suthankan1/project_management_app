...
export function WorkloadMembersList({ 
  ...
  return (
    <div className="w-full lg:w-8/12 p-6 relative" onMouseLeave={() => setActiveIndex(-1)}>
            ...
            >
              ...
              <div className="flex flex-col sm:items-end gap-2.5">
                <div className="flex items-center gap-2">
                  {member.overdue > 0 && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full font-extrabold tracking-wider bg-cu-danger/10 backdrop-blur text-cu-danger border border-cu-danger/20 uppercase whitespace-nowrap shadow-cu-sm">
                      {member.overdue} OVERDUE
                    </span>
                  )}
                  <span className="flex items-center gap-1.5 text-[12px] font-arimo font-semibold text-cu-text-primary bg-cu-bg-secondary px-2.5 py-0.5 rounded-lg border border-cu-border shadow-cu-sm backdrop-blur-sm">
                ...
                <div className="flex items-center gap-3 w-full sm:w-[140px]">
                  <span className="text-[11px] font-bold whitespace-nowrap" style={{ color: member.value > 0 && member.completed === member.value ? 'var(--cu-success)' : member.color }}>
                    {member.value > 0 ? Math.round((member.completed / member.value) * 100) : 0}%
                  </span>
                  <div className="h-2 w-full bg-cu-bg-tertiary rounded-full overflow-hidden shadow-inner flex-1 border border-cu-border-light">
                    <div
      ...
      <div className="absolute bottom-6 left-6 right-8 h-16 bg-gradient-to-t from-cu-bg via-cu-bg/40 to-transparent pointer-events-none z-20 rounded-b-xl backdrop-blur-[1px]" />
    </div>
  );
}
