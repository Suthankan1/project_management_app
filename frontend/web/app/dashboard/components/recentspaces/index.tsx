...
export default function RecentSpacesSection({ projects, loading }: RecentSpacesSectionProps) {
  ...
  return (
    <div className="flex flex-col gap-4 pb-[0.8px] bg-cu-bg relative mt-1">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3 w-full mt-1">
        ...
        <div className="flex justify-between items-center w-full md:w-auto px-1 h-5">
          <h2 className="font-outfit text-[15px] font-bold text-cu-text-primary m-0 flex items-center h-full">
            Recent spaces
          </h2>
          <Link
            href="/spaces"
            className="md:hidden font-outfit text-[13px] font-bold text-cu-primary hover:text-cu-primary-hover m-0 flex items-center h-full leading-none"
          >
            View all
          </Link>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full md:w-auto">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
            ...
            <div className="relative w-full sm:w-[220px]">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <svg className="text-cu-text-muted" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <input
                ...
                className="block w-full pl-9 pr-3 py-1.5 border border-cu-border rounded-[6px] leading-5 bg-cu-bg placeholder-cu-text-muted text-cu-text-primary focus:outline-none focus:ring-1 focus:ring-cu-primary/30 focus:border-cu-primary text-[13px] font-outfit shadow-cu-sm"
              />
            </div>

            {/* Recent / Favourites Tab Switcher */}
            <div className="flex items-center bg-cu-bg-secondary p-1 rounded-lg sm:bg-transparent sm:p-0 gap-1 w-full sm:w-auto mt-1 sm:mt-0">
              <button
                ...
                className={`flex-1 sm:flex-none px-3 py-1.5 rounded-[6px] font-outfit text-[11px] font-bold uppercase tracking-wider transition-all ${
                  recentFilter === 'recent'
                    ? 'bg-cu-bg text-cu-primary shadow-sm border border-cu-border'
                    : 'text-cu-text-secondary hover:text-cu-primary hover:bg-cu-hover'
                }`}
              >
                Recent
              </button>
          ...
          <Link
            href="/spaces"
            className="hidden md:block font-outfit text-[13px] font-bold text-cu-primary hover:text-cu-primary-hover ml-2 shrink-0"
          >
            View all
          </Link>
        </div>
