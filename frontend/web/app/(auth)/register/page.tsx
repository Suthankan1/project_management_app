    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 font-sans">
      ...
      <AuthCard>
        ...
        <form onSubmit={handleRegister} className="space-y-4" noValidate>
          ...
          {error && (
            <div id="register-error" role="alert" aria-live="polite" className="bg-cu-danger/10 border border-cu-danger/30 text-cu-danger px-4 py-3 rounded-lg text-sm">
              {error}
            </div>
          )}
              ...
              <div id="pw-strength" className="mt-2" aria-label={`Password strength: ${STRENGTH_LABELS[strength]}`}>
                <div className="flex gap-1 mb-1">
                  {[1, 2, 3, 4].map((seg) => (
                    <div
                      key={seg}
                      className={`h-1.5 flex-1 rounded-full transition-colors ${strength >= seg ? STRENGTH_COLOURS[strength] : 'bg-cu-bg-tertiary'}`}
                    />
                  ))}
                </div>
                <p className="text-xs text-cu-text-muted mb-2">{STRENGTH_LABELS[strength]}</p>
