...
import {
  CreateChannelModal, EditMessageModal,
  ConfirmDeleteModal, EditChannelModal,
} from '@/src/components/chat/ChatModals';
...
export function ChatTabContent({ projectId, navHeight }: Props) {
  ...
  const handleSelectUser = (u: string | null) => { selectPrivateUser(u); setShowSidebar(false); };
  const handleSelectRoom = (id: number | null) => { selectRoom(id); if (id !== null) setShowSidebar(false); };
  ...
  if (isLoading) return <View style={{ paddingTop: navHeight, flex: 1 }}><ChatLoadingSkeleton /></View>;

  return (
    <View style={s.root}>
      {showSidebar ? (
        <View style={[s.sidebarWrapper, { paddingTop: navHeight }]}>
          <ChatSidebar
            ...
            onEditRoom={(room: ChatRoom) =>
              setEditingRoom({ id: room.id, name: room.name, topic: room.topic || '', description: room.description || '' })
            }
      ...
      {editingRoom && (
        <EditChannelModal
          ...
          onSave={(updates) => { updateRoomMeta(editingRoom.id, updates); setEditingRoom(null); }}
        />
      )}
      <ThreadBottomSheet
...
const s = StyleSheet.create({
  ...
  sidebarWrapper: { flex: 1, backgroundColor: Colors.pageBg },
